...
@RequestMapping("/admin") //localhost:8090/admin >>>해당 라우터 경로 아래로는 이클래스의 처리를 담당한다
public class AdminController {
    private final NewsService newsService;

    @GetMapping("/category")
    public String categories(Model model){
        // 데이터 베이스로부터 카테고리 정보를 가져와서 admin 의 category페이지에 전달한다
    ...
    public String inputCategory(@RequestParam("category_name")String categoryName,Model model){
        //categoryName이 null이 아니면
        //.trim() 은 공백을 제외하고 문자열만 잘라주는 문장
        if(categoryName !=null && !categoryName.trim().isEmpty()){
            //카테고리명 데이터가 정상적으로 전달됨
// category entity  인스턴스를 생성
            ...
            if(msg!=null &&msg.startsWith("error")){
                ...
                //templates 폴더 아래에 있는 category.html을 랜더링 해라
